...
## 3c. Phase 6 — Loop Filters : le calme avant la tempête

Phase 6 (deblocking + SAO) s'est déroulée sans les drames de Phase 4/5. Traitement d'image pur, pas de CABAC, pas de propagation d'erreur. Un seul bug notable : le `continue` luma qui sautait aussi le chroma dans le deblocking.

**Jalon majeur** : `oracle_full_qcif_10f` pixel-perfect = **Main profile complet**. BBB 1080p/4K passent aussi. Seule limitation : multi-slice (3 tests échouent).

---

## 3d. Phase 7 — Main 10 : le bug dormant

### Jour 4 — 22 mars 2026 (après-midi) : Main 10 pixel-perfect

**Surprise** : l'architecture 10-bit était déjà prête. La décision AD-002 (`Pixel = uint16_t`) prise en Phase 1 a porté ses fruits :
- Aucun `255` hardcodé — tous les clips utilisent `(1 << bitDepth) - 1`
- `QpBdOffsetY`/`QpBdOffsetC` correctement utilisés partout (dequant, deblocking beta/tC, chroma QP)
- Shifts d'interpolation paramétrés par `bitDepth`
- YUV output 10-bit déjà géré dans `write_yuv()`

L'audit complet du pipeline (transform, deblocking, SAO, interpolation, weighted prediction) n'a révélé **aucun** problème 10-bit. Le I-frame 10-bit est passé pixel-perfect du premier coup.

### Bug n°18 — Le bug dormant depuis Phase 5 (CRITIQUE)

**Symptôme** : I-frame 10-bit pixel-perfect, mais P-frame 10-bit complètement faux. max_diff=301, 5959 pixels différents sur un frame 64x64.

**Investigation** : Le P-frame à QP 45 est 100% skip CUs (tous les pixels = motion compensation sans résidu). HM et ffmpeg produisent un P-frame identique au I-frame (MV=0 partout). Notre décodeur produit des valeurs complètement différentes.

**La traque** : Trace CABAC → notre décodeur parse MV=(7,0) au lieu de MV=(0,0). Mais le CU est skip ! Le skip mode n'a pas de MVD — il utilise merge seulement. Comment peut-on parser un MVD pour un CU skip ?

**Root cause** : `decode_prediction_unit_inter()` lit `cu.pred_mode` du **CU grid** pour décider entre merge (skip) et AMVP. Mais `pred_mode` est stocké dans le grid à la ligne 691, **APRÈS** l'appel à `decode_prediction_unit_inter` à la ligne 553. Résultat : le grid a encore `MODE_INTRA` (default), la fonction entre dans le chemin AMVP, lit `merge_flag` (bin fantôme) + `mvd` du bitstream → parsing complètement décalé.

**Pourquoi masqué en 8-bit pendant toute Phase 5/6** : les bitstreams de test 8-bit (preset medium, QP 22) n'avaient soit pas de CUs skip, soit les CUs skip tombaient dans des états CABAC où le bin `merge_flag` fantôme valait 1 par hasard, produisant le même résultat que le chemin correct. Le bitstream 10-bit (ultrafast, QP 45 = 100% skip sur fond intra) a forcé le bug dans 100% des cas.

**Fix** : 5 lignes — stocker `pred_mode = MODE_SKIP` dans le CU grid AVANT l'appel à `decode_prediction_unit_inter`.

**Enseignement n°21 — Les bugs latents se révèlent quand on change de profil** : le passage 8-bit → 10-bit n'a pas trouvé de bug dans le pipeline 10-bit (tout était déjà correct). Il a trouvé un bug dans le pipeline 8-bit qui était masqué par les bitstreams de test. La vraie valeur du 10-bit, c'est d'exercer des chemins de code différents (QP élevé, plus de skip CUs).

### Bug n°19 — Multi-frame YUV output (TRIVIAL)

Le chemin multi-frame dans `main.cpp` castait en `uint8_t` hardcodé. Le chemin single-frame (`write_yuv`) gérait correctement le 10-bit. Fix trivial + correction du chroma crop qui assumait 4:2:0 (`/2` hardcodé au lieu de `SubWidthC/SubHeightC`).

### Résumé Phase 7

| Métrique | Valeur |
|----------|--------|
| Tests pixel-perfect | 2/2 (I-frame + full pipeline 10-bit) |
| Bugs corrigés | 2 (#18 skip pred_mode race, #19 YUV output cast) |
| Lignes changées | ~30 (coding_tree.cpp, main.cpp, oracle_test.sh, CMakeLists.txt) |
| Non-régression | 122/126 (même 4 échecs multi-slice qu'avant) |

**La différence Phase 4 vs Phase 7** : Phase 4 a été une guerre de 30 bugs sur 2 jours. Phase 7 s'est résolue en 2h grâce à l'architecture AD-002 et au fix d'un seul bug dormant. L'investissement en architecture initiale (Pixel=uint16_t, clips paramétrés, shifts spec-conformes) a transformé une extension potentiellement massive en une quasi-non-opération.
...
| Mesure | Valeur |
|--------|--------|
| Total C++ (src/) | ~7 500 lignes |
| `src/decoding/` seul | ~4 200 lignes |
| Plus gros fichier | `coding_tree.cpp` (~1 200 lignes) |

### Tests

| Phase | Tests pass | Tests total |
|-------|-----------|-------------|
| 4 (Intra) | 12/13 | 1 echec = multi-slice |
| 5 (Inter) | 10/10 | - |
| 6 (Filters) | 11/14 | 3 echecs = multi-slice |
| 7 (Main 10) | 2/2 | - |
| **Total** | **122/126** | 4 echecs = multi-slice (limitation connue) |

### Bugs

| Phase | Bugs | Categorie dominante |
|-------|------|---------------------|
| 4 | 30 | CABAC contextes (10), intra (6), transform (5), structure (5), residual (4) |
| 5 | 3 | Flags ignores (1), ordering (1), conditions implicites (1) |
| 6 | 1 | Deblocking chroma skip |
| 7 | 2 | Race condition (1), output format (1) |
| **Total** | **36** | |
