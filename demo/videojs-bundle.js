  ...
  function createTranscodingProxy(realSB, config) {
    ...
    if (useWorker) {
      ...
      workerClient.waitReady().then(() => {
        ...
        console.error("[hevc.js] Worker init failed:", err?.message ?? err);
      });
    } else {
      transcoder = new SegmentTranscoder(config);
      transcoder.init().catch((err) => {
        console.error(
          "[hevc.js] Main-thread transcoder init failed:",
          err?.message ?? err,
          err?.stack
        );
      });
    ...
    async function processNext(target) {
      ...
      try {
        ...
        if (abortGeneration !== myGeneration) return;
        console.error(
          "[hevc.js/dash] Transcoding error:",
          err?.message ?? err,
          err?.stack
        );
        fakeUpdating = false;
